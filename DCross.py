...
class DCN:
	def __init__(self, u_input_dims, i_input_dims, emb_dim, user_dims, item_dims, phase, nb_cross, params = None, lr = 1e-4, epsilon = 1e-6):
	...
	def _set_graph(self):
		...
		# Embedding
		self.u_h = [tf.matmul(self.inputs[i], self.u_emb[i-1]) for i in range(1, self.nb_u_in)]
		...
		#self.att = tf.nn.sigmoid(tf.matmul(self.inputs[0], self.att_param[0]))
		self.att = tf.matmul(self.inputs[0], self.att_param[0])
		...
		self.c_h = self.att + self.u_h
		# Cross layer
		for i in range(self.nb_cross):
			#self.c_h = tf.matmul(tf.matmul(tf.transpose(self.u_h), self.c_h), self.c_params[i][0])
			self.c_h = tf.add(self.u_h * tf.matmul(self.c_h, self.c_params[i][0]), self.c_params[i][1]) + self.c_h
			#self.c_h = tf.add(self.c_h, self.c_params[i][1]) + self.u_h

		# The answer
		self.ans_0 = tf.nn.sigmoid(tf.matmul(self.u_h, tf.transpose(self.i_h)))
		#self.pos_att = tf.multiply(self.u_h, self.att)
		self.ans_1 = tf.nn.sigmoid(tf.matmul(self.c_h, tf.transpose(self.i_h)))
	...
	def _set_weight(self, params):
		...
		# Embedding
		if params:
			self.u_emb = [tf.Variable(params[0][i]) for i in range(self.nb_u_in-1)]
			self.i_emb = [tf.Variable(params[1][i]) for i in range(self.nb_i_in)]
			self.att_param = [tf.Variable(params[4][0])]
		else:
			self.u_emb = [tf.Variable(tf.random_normal([self.u_in_dim[i], self.emb_dim], 0.0, 0.01)) for i in range(1, self.nb_u_in)]
			self.i_emb = [tf.Variable(tf.random_normal([self.i_in_dim[i], self.emb_dim], 0.0, 0.01)) for i in range(self.nb_i_in)]
			self.att_param = [tf.Variable(tf.random_normal([self.u_in_dim[0], self.u_dims[-1]], 0.0, 0.01))]
		# DNN User
		for i in range(len(self.u_dims) - 1):
			if params:
		...
		for i in range(self.nb_cross):
			if params:
				W = tf.Variable(params[5][i][0])
				b = tf.Variable(params[5][i][1])
			else:
				W = tf.Variable(tf.random_normal([self.u_dims[-1], 1], 0.0, 0.01))
				b = tf.Variable(tf.random_uniform([self.u_dims[-1]], 0.0, 0.0))
			self.c_params.append([W, b])

	def save_weight(self, file_name):
		params = self.sess.run([self.u_emb, self.i_emb, self.u_params, self.i_params, self.att_param, self.c_params])
	...
	def get_feature(self, in_feat, end, att = True, batch_size = 2000):
		...
		for i in range(nb_batch):
			if end == 'user':
				...
				if att:
					ans = self.sess.run([self.c_h], feed_dict = {self.inputs[i]: i_f[i] for i in range(self.nb_u_in)})
				else:
